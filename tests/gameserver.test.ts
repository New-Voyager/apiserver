...
describe('Game server APIs', () => {
  ...
  test('create a game server', async () => {
    ...
    try {
      const resp = await axios.post(`${GAMESERVER_API}/register-game-server`,gameServer);
  ...
  test('update a game server', async () => {
    ...
    try {
      ...
      const updateResp = await axios.post(`${GAMESERVER_API}/update-game-server`,gameServerUpdate);
      expect(resp.status).toBe(200);
      expect(resp.data.status).toBe("OK");
    } catch(err) {
      console.error(JSON.stringify(err));

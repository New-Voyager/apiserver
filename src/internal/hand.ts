  ...
  public async saveHand(req: any, resp: any) {
    ...
    try {
      ...
      if (!registerPayload.Result) {
        ...
        if (
          !registerPayload.Result.pot_winners ||
          registerPayload.Result.pot_winners.length === 0
        ) {
    ...
    try {
      ...
      if (registerPayload.Result.showdown) {
        if (registerPayload.GameType === GameType[GameType.OMAHA_HILO]) {
          await registerPayload.Result.pot_winners[0].hi_winners.forEach(
            async (winner: {
          ...
          await registerPayload.Result.pot_winners[0].lo_winners.forEach(
            async (winner: {
              winning_cards: Array<string>;
              rank_num: number;
              player: string;
          ...
          await registerPayload.Result.pot_winners[0].winners.forEach(
            async (winner: {
              winning_cards: Array<string>;
              rank_num: number;
              player: string;

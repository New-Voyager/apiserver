...
class GameRepositoryImpl {
  public async createPrivateGame(clubId: string, playerId: string, input: any, template: boolean = false): Promise<PokerGame> {
    ...
    if(!(clubMember.isOwner || clubMember.isManager)) {
      throw new Error(`The player ${playerId} is not a owner or a manager of the club`);
    }

    if(clubMember.isManager && clubMember.status == ClubMemberStatus.APPROVED) {
      throw new Error(`The player ${playerId} is not an approved manager to create a game`);
    }
 
    // create the game
    let game: PokerGame = {...input} as PokerGame;
    let gameTypeStr: string = input["gameType"];
    let gameType: GameType = GameType[gameTypeStr];
